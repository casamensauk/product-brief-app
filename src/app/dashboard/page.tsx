    ...
    <div className="max-w-container-max mx-auto space-y-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <div className="lg:col-span-8 bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm flex flex-col">
          ...
          <div className="overflow-x-auto min-h-[300px]">
            <table className="w-full text-left border-collapse">
              <thead className="bg-surface-container-low">
                <tr>
                  <th className="px-lg py-3 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Project Name</th>
                  <th className="px-lg py-3 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Client</th>
                  <th className="px-lg py-3 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Status</th>
                  <th className="px-lg py-3 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Last Updated</th>
                  <th className="px-lg py-3 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
                ...
                {loading ? (
                  ...
                  briefs.map(brief => (
                    <tr key={brief.id} className="hover:bg-surface-container-low transition-colors group cursor-pointer" onClick={() => window.location.href = `/dashboard/brief/${brief.shareToken}`}>
                      ...
                      <td className="px-lg py-4 text-on-surface-variant">{brief.clientName || 'Unknown Client'}</td>
                      <td className="px-lg py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusStyle(brief.status)}`}>
                          {brief.status}
                        </span>
                      </td>
                      <td className="px-lg py-4 text-label-md text-on-surface-variant">
          ...
          <div className="mt-auto px-lg py-4 bg-surface-container-low flex items-center justify-between text-label-sm text-on-surface-variant">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-lg shadow-sm">
            ...
            <div className="space-y-md">
              {briefs.slice(0, 3).map((brief, index) => (
                <div key={`activity-${brief.id}`} className={`flex gap-3 ${index < 2 ? 'relative pb-md' : ''}`}>
            ...
            {briefs.length > 0 && <button className="w-full mt-lg text-secondary font-label-md hover:underline">View all activity</button>}
          </div>

          {/* Template Quick Access */}
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-lg shadow-sm">
            <h3 className="font-headline-sm text-headline-sm mb-md">Quick Templates</h3>
            <div className="grid grid-cols-2 gap-3">
              <button className="flex flex-col items-center justify-center p-md rounded-xl border border-surface-variant hover:border-secondary hover:bg-surface-container-high transition-all text-center gap-2 group">
